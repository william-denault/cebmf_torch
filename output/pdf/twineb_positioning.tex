...
downstream evidence described in \eqref{eq:child}.
...
The feature-factor assumptions differ as well. TwinEB's column prior is a
mixture on an entire feature's latent vector. The principal implementation here
uses independent ash priors for feature columns. Optional HMM priors instead
connect adjacent feature positions within each factor. Neither construction
is a strict superset of TwinEB's vector column prior. Our Gaussian observation
implementation also does not subsume the paper's Poisson formulation.

\paragraph{Inference and the proposed contribution.}
Both approaches jointly learn priors and variational posterior approximations.
Here the loading-coordinate posterior is profiled out, giving the local
log-normalizer objective in \eqref{eq:learning}. The no-latent-parent/no-child
limit is the original cEBNM marginal-likelihood update. Numerical integration
is needed for nonlinear child contributions, and a factorized posterior
does not represent posterior covariance between loading coordinates.

The methodological candidate is a conditional-prior factorization framework
with explicit activation probabilities, modality-specific programs, fixed
side information and a cEBMF-compatible learning algorithm. Autoregressive
decomposition and coordinate variational inference are established mathematics.
Novelty and scientific value must rest on the model construction, useful
inference and evidence at matched predictive and computational budgets.
...
spikes, fixed covariates and optional HMM smoothing. Exponential slabs remain a separate extension.
