...
from cebmf_torch.utils.batching import density_batches
...
class Cgb2PosteriorResult:
    def __init__(
        ...
        mu_pos : scalar torch.Tensor
            Global mean of the positive slab (>= 0).
        mu_neg : scalar torch.Tensor
            Global mean of the negative slab (<= 0).
        sigma_1 : scalar torch.Tensor
            Global SD of the positive slab.
        sigma_2 : scalar torch.Tensor
...
def sharp_2cgb_posterior_means(
    ...
    penalty : float, optional
        Penalty for spike probability pi_0 (default=1.1). Values > 1 enforce
    ...
    dataloader = density_batches(dataset, batch_size, shuffle=True)
    ...
    optimizer = optim.Adam(model.parameters(), lr=lr, capturable=betahat.is_cuda)
    ...
    return Cgb2PosteriorResult(
        ...
        pi_pos=pi1,
        pi_neg=pi2,
        mu_pos=mu_pos.detach(),
        mu_neg=mu_neg.detach(),
        sigma_1=sigma1_sq.sqrt().detach(),
        sigma_2=sigma2_sq.sqrt().detach(),
        loss=-log_marginal,
