...
def cash_posterior_means(
    ...
    optimizer_cash = optim.Adam(model_cash.parameters(), lr=lr, capturable=betahat.is_cuda)
    ...
    model_cash.eval()
    with torch.no_grad():
        train_loader = density_batches(dataset, len(dataset), shuffle=False)
        ...
        full_marginal_ll = log_marginal_per_obs.sum()
