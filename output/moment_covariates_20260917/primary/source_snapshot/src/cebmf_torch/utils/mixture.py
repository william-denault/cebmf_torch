...
def optimize_pi_logL(
    ...
    check_every : int, optional
        How many EM steps to run between convergence checks. Convergence is
        recorded on-device; subsequent updates keep the converged solution.
    ...
    eps_floor = 1e-12
    done = torch.zeros((), device=device, dtype=torch.bool)
    converged_iter = torch.full((), -1, device=device, dtype=torch.int64)
    ...
    for it in range(max_iters):
        ...
        pi = torch.where(done, pi_old, n_k / n_k.sum())

        # Preserve exactly the first iterate accepted by the stopping rule,
        # without reading a CUDA convergence flag back to Python.
        if (it + 1) % check_every == 0:
            converged = torch.linalg.norm(pi - pi_old) < tol
            converged_iter = torch.where(~done & converged, it, converged_iter)
            done = done | converged

    if verbose and int(converged_iter) >= 0:
        print(f"Converged after {int(converged_iter) + 1} iterations.")

    return pi


def _calculate_scales(
    sigmaamax: float | torch.Tensor,
    sigmaamin: float | torch.Tensor,
    mult: float,
    device: torch.device,
    dtype: torch.dtype,
) -> torch.Tensor:
    ...
    """
    sigmaamax = torch.as_tensor(sigmaamax, device=device, dtype=dtype)
    sigmaamin = torch.as_tensor(sigmaamin, device=device, dtype=dtype)
    # Only the dynamic array length crosses to the host. Grid amplitudes stay
    # on-device. An explicit ASH scale grid bypasses this shape decision.
    npoint = int(torch.ceil(torch.log2(sigmaamax.double() / sigmaamin.double()) / math.log2(mult)))
...
def autoselect_scales_mix_norm(betahat: torch.Tensor, sebetahat: torch.Tensor, max_class=None, mult: float = 2.0):
    ...
    scales = _calculate_scales(sigmaamax, sigmaamin, mult, device, dtype)
...
def autoselect_scales_mix_exp(
    ...
    if max_class is not None:
        if scales.numel() != max_class:
            scales = torch.linspace(scales.min(), scales.max(), steps=max_class, device=device, dtype=dtype)
            if scales.numel() >= 3:
                scales[2:] = scales[2:] + torch.where(scales[2] < 1e-2, 1e-2, 0.)
    return scales
