...
class ASHBuilder(PriorBuilder):
    ...
    def fit(
        ...
        penalty = torch.as_tensor(self.kwargs.get("penalty", 10.0), device=obj.pi.device, dtype=obj.pi.dtype)
        if penalty.ndim == 0:
            regularization = (penalty - 1) * obj.pi[0].clamp_min(1e-30).log()
        else:
            regularization = ((penalty - 1) * obj.pi.clamp_min(1e-30).log()).sum()
