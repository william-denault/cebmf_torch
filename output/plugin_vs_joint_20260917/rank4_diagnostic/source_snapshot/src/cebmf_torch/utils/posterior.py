...
def posterior_point_mass_normal(
    ...
    mu0: float,
    mu1: float | torch.Tensor,
    sigma_0: float | torch.Tensor,
):
    """
    ...
    sigma0 = torch.as_tensor(sigma_0, dtype=x.dtype, device=x.device).clamp_min(1e-8)
    se = torch.clamp(se, min=1e-8)

    # marginal likelihoods
    log_slab = _logpdf_normal(
        x,
        torch.as_tensor(mu1, dtype=x.dtype, device=x.device),
        torch.sqrt(se**2 + sigma0**2),
    ) + torch.log1p(-pi)
    log_spike = _logpdf_normal(x, torch.as_tensor(mu0, dtype=x.dtype, device=x.device), se) + pi.log()

    w0 = (log_spike - torch.logaddexp(log_spike, log_slab)).exp()
    w1 = 1.0 - w0
