...
from .learning import fit_mixture_network, spike_log_factor
...
class ConditionalLoadingSampler:
    ...
    def child_log_prior(self, u: int, candidate: Tensor) -> Tensor:
        ...
        for child in range(u + 1, len(self.priors)):
            ...
            result += spike_log_factor(mixture, getattr(self, "penalty", 1.0))
        return result
    ...
    def loading_log_conditional(self, u: int, value: Tensor, component: Tensor, a: Tensor, b: Tensor) -> Tensor:
        ...
        return (-0.5 * a * value.square() + b * value + own.log_prob(value, component)
                + spike_log_factor(own, getattr(self, "penalty", 1.0)) + self.child_log_prior(u, value))
    ...
    def _fit_from_draws(self, values: Tensor, components: Tensor, *, steps: int, lr: float):
        ...
        for u, prior in enumerate(self.priors):
            ...
            settings = getattr(self, "training", {})
            round_record.append(fit_mixture_network(
                prior, x, v, components=z,
                n_epochs=settings.get("n_epochs", steps), batch_size=settings.get("batch_size"),
                lr=settings.get("lr", lr), penalty=getattr(self, "penalty", 1.0),
