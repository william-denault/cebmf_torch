...
def _fit_lcash(
    ...
    optimizer = torch.optim.Adam(param_groups, lr=lr, capturable=betahat.is_cuda)
    ...
    # by `cebmf.py`'s per-factor `kl_l[k] = nm_ll_L + loss` formula, which
    # evaluates to the non-negative KL(q(L_k) || p(L_k)).
