...
def _ash_normal(
    ...
    scale = config.scales if config.scales is not None else autoselect_scales_mix_norm(x, s, mult=config.mult)
...
def _ash_exp(
    ...
    scale = config.scales if config.scales is not None else autoselect_scales_mix_exp(x, s, mult=config.mult)
...
def ash(
    ...
    *,
    ...
    threshold_loglikelihood : float, optional
        Retained for compatibility; the likelihood is not clipped because
        clipping would break the normal-means posterior/ELBO identity.
    mode : float, optional
    ...
    scales : torch.Tensor or None, optional
        Fixed nonnegative SD grid, starting with zero, on the input device.
        Mixture weights are still refitted on every call. A fixed grid avoids
        the host scalar needed to allocate an automatically sized grid.
    ...
    if scales is not None:
        scales = torch.as_tensor(scales, device=x.device, dtype=x.dtype)
        if scales.ndim != 1 or len(scales) < 2:
            raise ValueError("scales must be a vector with at least two entries.")
        require_finite(scales, "scales must be finite.", ValueError)
        valid = (scales[0] == 0) & (scales[1:] > scales[:-1]).all()
        if scales.is_cuda:
            torch._assert_async(valid, "scales must start at zero and strictly increase.")
        elif not valid:
            raise ValueError("scales must start at zero and strictly increase.")
