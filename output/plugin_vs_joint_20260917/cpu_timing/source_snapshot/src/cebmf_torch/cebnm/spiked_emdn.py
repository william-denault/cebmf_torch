...
def mdn_spike_loss_with_varying_noise(
    ...
    *,
    ...
    dist_slab = torch.distributions.Normal(mu, total_sigma_slab, validate_args=False)
...
class EmdnPosteriorMeanNorm:
    def __init__(
        ...
        loss : scalar torch.Tensor, optional
            Final training loss.
...
def spiked_emdn_posterior_means(
    ...
    *,
    ...
    # ---- predict (all data)
    model.eval()
    full_loader = density_batches(dataset, len(dataset), shuffle=False)
    with torch.no_grad():
