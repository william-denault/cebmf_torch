...
class LearnedBuilder(PriorBuilder):
    ...
    def set_kwargs(self, **new_kwargs: Any) -> None:
        ...
        **new_kwargs : Any
        ...
        """
        if "omega" in new_kwargs and self.type == LearnedPriorType.CGB:
    ...
    def fit(
        ...
        if X is None and self.type not in (LearnedPriorType.HMM, LearnedPriorType.HMM_POS, LearnedPriorType.HMM_NEG):
            X = betahat.new_ones((len(betahat), 1))
