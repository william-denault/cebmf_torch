...
def _impute_nan(Y: Tensor) -> Tensor:
    ...
    """
    # Entirely unobserved columns carry no information; initialize them at
    # zero. torch.where also avoids data-dependent indexing on CUDA.
    col_means = torch.nanmean(Y, dim=0).nan_to_num(nan=0.0)
    return torch.where(torch.isnan(Y), col_means[None, :], Y)
...
def svd_initialise(Y: Tensor, N: int, P: int, K: int, device: torch.device) -> tuple[Tensor, Tensor]:
    ...
    if K_actual < K:
        L = torch.cat([L, Y.new_zeros(N, K - K_actual)], dim=1)
        F = torch.cat([F, Y.new_zeros(P, K - K_actual)], dim=1)
...
def random_initialise(Y: Tensor, N: int, P: int, K: int, device: torch.device) -> tuple[Tensor, Tensor]:
    ...
    tuple of Tensor
        Tuple of (L, F) tensors
    """
    L = torch.randn(N, K, device=device, dtype=Y.dtype) * RANDOM_INIT_SCALE
    F = torch.randn(P, K, device=device, dtype=Y.dtype) * RANDOM_INIT_SCALE
    return L, F
...
def zero_initialise(Y: Tensor, N: int, P: int, K: int, device: torch.device) -> tuple[Tensor, Tensor]:
    ...
    tuple of Tensor
        Tuple of (L, F) tensors
    """
    L = torch.zeros(N, K, device=device, dtype=Y.dtype)
    F = torch.zeros(P, K, device=device, dtype=Y.dtype)
    return L, F
