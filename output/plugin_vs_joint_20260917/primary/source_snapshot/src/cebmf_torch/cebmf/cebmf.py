...
class CEBMFResult:
    ...
    tau : Tensor
        Noise precision(s).
    history_obj : list
        Objective values as scalar tensors on the model device. Convert for
        plotting/JSON explicitly, e.g. torch.stack(history_obj).cpu().tolist().
    """
    ...
    elbo_estimate: object | None = None
    history_elbo: list | None = None
    objective_kind: str = "negative_elbo"
...
class cEBMF:
    ...
    - User-supplied (fixed) standard errors via ``S`` â€” useful for z-scores
    ...
    def __init__(
        ...
        conditional_kwargs: dict | None = None,
        verbose: bool = True,
    ):
        """
        Parameters
        ...
        prune_thresh : float, optional
            Ï€0 threshold above which a factor is pruned.
        ...
        S : torch.Tensor, float, int, or None, optional
            ...
            * a scalar â€” typical for z-scores (``S=1.0``) â€” applied to every
              entry;
            * a tensor broadcastable to ``data.shape`` â€” for example, ``(N, P)``
        ...
        self_row_cov, self_col_cov : bool, optional
            ...
            Effective flags select conditional variational inference, including
            uncertainty in earlier factors and child-prior contributions.
            HMM priors ignore the flag on their axis.
        device : torch.device or None, optional
            Target device. Defaults to the result of :func:`get_device`.
            Conditional variational fitting uses the model's device.
        joint_kwargs : dict or None, optional
            Deprecated explicit controls for the experimental reference sampler.
            Omit this argument for conditional variational inference.
        conditional_kwargs : dict or None, optional
            approximation="quadratic" (default) uses cached local quadratic
            child feedback and analytic component updates for faster fitting.
            A warning at fit setup explains the approximation. Set
            approximation="quadrature" to use quadrature child feedback instead.
            Parent integration controls: quadrature_points (24),
            parent_samples (32) and seed (0). Training options remain in
            prior_L_kwargs/prior_F_kwargs. No burn-in or posterior chain is used.
        verbose : bool, optional
            Print a short completion message after each variational sweep
            (default True). Set False to silence sweep progress. This does not
            suppress warnings or explicitly enabled inner-prior logging.
            Messages use a Python counter and do not read GPU tensors.
        """
        self.data = data
        ...
        # Move covariates to device (if provided) to avoid later CPUâ†”GPU hops
        ...
        data_on_device = self.data.to(self.device)
        self.Y = data_on_device if data_on_device.dtype.is_floating_point else data_on_device.float()
        self.data = self.Y  # do not retain a second, host-side observation matrix
        self.N, self.P = self.Y.shape
        prior_L_kwargs = {key: value.to(self.device) if isinstance(value, Tensor) else value
                          for key, value in prior_L_kwargs.items()}
        prior_F_kwargs = {key: value.to(self.device) if isinstance(value, Tensor) else value
                          for key, value in prior_F_kwargs.items()}
        ...
        self.conditional_kwargs = dict(conditional_kwargs or {})
        self.conditional_fit = None
        if joint_kwargs:
            warn("joint_kwargs explicitly selects the deprecated experimental sampler. "
                 "Omit it to use the conditional variational update; numerical controls "
                 "belong in conditional_kwargs.", FutureWarning, stacklevel=2)
    ...
    def fit(self, maxit: int = 50):
        ...
        ----------
        maxit : int, optional
            Number of variational sweeps. Each sweep updates loading priors,
            loading moments, feature priors/moments and unknown noise.
            Conditional graphs retain their starting rank and ordering.

        Returns
        -------
        CEBMFResult
            Fitted factors, noise, and objective history. Conditional fits
            return inference="conditional_quadratic" by default, or
            "conditional_variational" with approximation="quadrature". Both
            report a numerical approximation to the negative regularized ELBO
            (lower is better).
            Numerical integration and finite neural optimization do not
            guarantee a monotone history. Explicit legacy joint_kwargs retain
            the deprecated sampler's separate behavior.
        """
        ...
        if self._uses_conditional_inference:
            if isinstance(maxit, bool) or not isinstance(maxit, int) or maxit < 0:
                raise ValueError("maxit must be a nonnegative integer.")
            self._ensure_conditional_fit()
            if len(self.conditional_fit.models) != 1:
                raise ValueError("This model belongs to a coupled fit; continue with fit_joint(atac, rna).")
            for _ in range(maxit):
                self.conditional_fit.step()
            self.conditional_fit.finish()
        ...
        if self._uses_joint_inference:
            ...
            self.obj = [estimate.loss for estimate in self.joint_sampler.elbo_history]
            return CEBMFResult(
                self.L, self.F, self.tau, self.obj, result.reconstruction, result, "joint",
                result.elbo, list(self.joint_sampler.elbo_history), "negative_regularized_elbo_mc",
            )
        for _ in range(maxit):
            self.iter_once()
    ...
    def initialise_factors(self, method: str = "svd", *, L: Tensor | None = None, F: Tensor | None = None):
        ...
        self.L, self.F = self.L.to(self.Y), self.F.to(self.Y)
    ...
    def iter_once(self):
        """
        Perform one variational sweep, including conditional-prior learning.

        Only an explicitly selected legacy sampler uses a fixed-parameter draw.
        """
        if self._uses_conditional_inference:
            self._ensure_conditional_fit()
            if len(self.conditional_fit.models) != 1:
                raise ValueError("Continue a coupled model with fit_joint(atac, rna).")
            self.conditional_fit.step()
            return
        if self._uses_joint_inference:
            self._ensure_joint_sampler()
            ...
            self.joint_posterior = None
            self.obj.append(self.joint_sampler.record_elbo().loss)
            self._report_sweep()
            return
    ...
    def _report_sweep(self, *, joint=False, display=True):
        """Count completed sweeps without reading or synchronizing device tensors."""
        # getattr also supports checkpoints created before verbosity existed.
        self._sweeps_completed = getattr(self, "_sweeps_completed", 0) + 1
        if display and getattr(self, "verbose", True):
            label = "cEBMF joint" if joint else "cEBMF"
            print(f"{label} sweep {self._sweeps_completed} completed.", flush=True)

    @property
    def _uses_joint_inference(self) -> bool:
        return self.joint_sampler is not None or (bool(self.joint_kwargs) and
            (self.covariate.self_row_cov or self.covariate.self_col_cov))

    @property
    def _uses_conditional_inference(self) -> bool:
        if self.conditional_fit is not None:
            return True
        if self.joint_sampler is not None or self.joint_kwargs:
            return False
        from cebmf_torch.priors.conditional import SCALAR_PRIORS
        return self.model.K > 1 and (
            (self.covariate.self_row_cov and self.model.prior_L in SCALAR_PRIORS)
            or (self.covariate.self_col_cov and self.model.prior_F in SCALAR_PRIORS))

    def _ensure_conditional_fit(self):
        if self.conditional_fit is None:
            from ._conditional import ConditionalFit
            self.conditional_fit = ConditionalFit([self], self.conditional_kwargs)

    def _conditional_result(self):
        return CEBMFResult(self.L, self.F, self.tau, self.obj, self.L @ self.F.T,
                           inference=("conditional_quadratic" if self.conditional_fit.options.get("approximation") == "quadratic"
                                      else "conditional_variational"),
                           history_elbo=[-v for v in self.obj],
                           objective_kind="negative_regularized_elbo_quadrature")

    @torch.no_grad()
    def _ensure_joint_sampler(self):
        from cebmf_torch.experimental.compat import ensure_joint_sampler
        ensure_joint_sampler(self)
    ...
    def _update_L_factor(self, k: int, Rk: Tensor, tau_map: Tensor | None, eps: float) -> None:
        ...
        if isinstance(resL.regularization, Tensor):
            self.regularization_L[k].copy_(resL.regularization)
        else:
            self.regularization_L[k].fill_(resL.regularization)
        ...
        # KL(q(L_k) || p(L_k)) â€” non-negative. By the EBNM identity, when q is the
        # exact posterior under the fitted prior Ï€Ì‚,
        #   log p(lhat | s, Ï€Ì‚) = E_q log N(lhat | Î¸, sÂ²) âˆ’ KL(q || p)
        # so KL = nm_ll_L âˆ’ log_lik = nm_ll_L âˆ’ (âˆ’resL.loss) = nm_ll_L + resL.loss.
        # This is the value the âˆ’ELBO accumulator in `_cal_obj` adds to âˆ’ll.
    ...
    def _update_F_factor(self, k: int, Rk: Tensor, tau_map: Tensor | None, eps: float) -> None:
        ...
        if isinstance(resF.regularization, Tensor):
            self.regularization_F[k].copy_(resF.regularization)
        else:
            self.regularization_F[k].fill_(resF.regularization)
        ...
        # KL(q(F_k) || p(F_k)) â€” see the matching note in `_update_L_factor`.
    ...
    def _cal_obj(self):
        if self.conditional_fit is not None:
            self.obj.append(self.conditional_fit.objective())
            return
        ...
        loss = -ll + KL  # keep scalar diagnostics on-device until the user reports them
    ...
    def _compute_elementwise_loglik(self, ER2: Tensor) -> Tensor:
        obs = self.mask.bool()
        c2pi = ER2.new_tensor(math.log(2.0 * math.pi))
        return -0.5 * (c2pi * obs.sum() - torch.where(obs, self.tau_map.log(), 0.).sum()
                       + torch.where(obs, self.tau_map * ER2, 0.).sum())
    ...
    def _expected_residuals_squared(self):
        ...
        first_moment_sq = (self.L.pow(2)) @ (self.F.pow(2)).T  # Î£_k (E[L]^2)(E[F]^2)^T
        second_moment = self.L2 @ self.F2.T  # Î£_k E[L^2] E[F^2]^T
    ...
    def _initialise_tensors(self):
        ...
        self.regularization_L = torch.zeros_like(self.kl_l)
        self.regularization_F = torch.zeros_like(self.kl_f)
    ...
    def _setup_known_variance(self) -> None:
        ...
        self.S = S
        self._S_input = S  # retain the normalized device tensor, not the caller's host copy
    ...
    def _should_prune_factor(self, k: int) -> bool:
        """
        Remove factor k if we have Ï€â‚€ info and the smallest Ï€â‚€ across coordinates
        (for any side that provided it) is â‰¥ thresh. (Your spec: use the *lowest* Ï€â‚€.)
        """
        pi0_min_L = safe_tensor_to_float(self.pi0_L[k])
        pi0_min_F = safe_tensor_to_float(self.pi0_F[k])
        # if neither side provided Ï€0, don't prune
    ...
    def _prune_indices(self, idxs: list[int]) -> None:
        ...
        if self.conditional_fit is not None:
            raise ValueError("Rank is fixed for a conditional loading graph; initialize a new fit to change it.")
        ...
        self.regularization_L = self.regularization_L[keep]
        self.regularization_F = self.regularization_F[keep]
