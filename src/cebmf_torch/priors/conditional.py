"""Conditional Gaussian-mixture distributions shared by inference backends."""
...
class GaussianMixture:
    ...
    def posterior(self, a: Tensor, b: Tensor):
        """Normal-means mixture and log integral of exp(-a*vÂ²/2+b*v).

        The formula avoids subtracting muÂ² / variance, works at zero variance
...
class ConditionalMixture(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        scales=None,
        omega: float = 0.01,
        slab_sd: float | Tensor = math.sqrt(0.05),
        min_sd: float = 0.02,
        max_sd: float = 20.0,
        ...
        slab_sd = torch.as_tensor(slab_sd, device=reference_inputs.device, dtype=reference_inputs.dtype)
        if not 0 < min_sd < max_sd or not 0 < omega <= 1 or not torch.isfinite(slab_sd) or slab_sd <= 0:
        ...
        if name in ("cgb", "cgb_sharp", "cgb_sharp_2"):
            ...
            sd = (slab_sd * (math.sqrt(omega) if sharp else 1)).clamp_min(min_sd)
            learn_scales = not sharp if learn_scales is None else learn_scales
            self.log_slab_sd = nn.Parameter(sd.log().expand(count).clone(), requires_grad=learn_scales)
            if count == 1:
                with torch.no_grad():
            ...
            with torch.no_grad():
                ...
                self.net.mu.bias.copy_(torch.linspace(0, 1, self.net.mu.out_features, device=x.device))
                self.net.log_sigma.weight.zero_()
                bias = slab_sd.log() if name == "emdn" else slab_sd + torch.log(-torch.expm1(-slab_sd))
                self.net.log_sigma.bias.copy_(bias.expand_as(self.net.log_sigma.bias))
    ...
    def standardize(self, inputs: Tensor):
        ...
        require_finite(inputs, "Latent parents and fixed covariates must be finite; missing observations use a mask.",
                       ValueError)
    ...
    def forward(self, inputs: Tensor) -> GaussianMixture:
        ...
        for value in (log_weight, mean, variance):
            require_finite(value, "Nonfinite conditional mixture; reduce learning rate or check input scaling.")
