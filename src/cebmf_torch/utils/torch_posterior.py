...
@torch.no_grad()
def posterior_mean_exp(betahat: torch.Tensor, sebetahat: torch.Tensor, log_pi: torch.Tensor, scale: torch.Tensor) -> PosteriorMeanExp:
    """
...
def posterior_point_mass_normal(
    ...
    mlik = _logpdf_normal(x, torch.tensor(mu1, dtype=x.dtype, device=x.device), torch.sqrt(se**2 + sigma_0**2)).exp()
