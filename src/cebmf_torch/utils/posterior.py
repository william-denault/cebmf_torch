...
def posterior_mean_exp(
    ...
    # E[theta_c | data] using truncnorm helpers (Torch)
    # m_tilt = x - s^2 * a
    for i in range(J):
        x_i = betahat[i]
        s_i = sebetahat[i]
        ...
        e1 = my_etruncnorm(zero, pinf, m_tilt, s_i)
        e2 = my_e2truncnorm(zero, pinf, m_tilt, s_i)
        ...
        post_mean2[i] = torch.maximum(post_mean2[i], post_mean[i])  # guard
...
def posterior_mean_norm(
    ...
    # locations (broadcast to JxK)
    loc = location.unsqueeze(0).expand(J, K) if location.ndim == 1 else location

    # avoid 0-division in loc/t2 by guarding where t2==0
    loc_over_t2 = torch.where(t2 > 0, loc / t2, torch.zeros_like(loc))
    rhs = t_ind_Var * (betahat.unsqueeze(1) / s2 + loc_over_t2)
